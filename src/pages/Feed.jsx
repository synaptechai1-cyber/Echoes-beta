...
export default function Feed() {
  ...
  async function handleFollow(post) {
    try {
      const res = await api.follow(post.username)
      setFollowing(prev => ({ ...prev, [post.username]: res.following }))
      setAnnouncement(res.following ? `Following ${post.display_name || post.username}` : `Unfollowed ${post.display_name || post.username}`)
    } catch {}
  }
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
            <button
              ...
              style={{ fontSize: 14, color: 'var(--text-2)', fontWeight: 500, textAlign: 'left' }}
            >
              by {current.display_name || current.username}
            </button>

            {current.username && echoUser?.username !== current.username && (
              <button
                onClick={() => handleFollow(current)}
                aria-pressed={!!following[current.username]}
                aria-label={following[current.username]
                  ? `Unfollow ${current.display_name || current.username}`
                  : `Follow ${current.display_name || current.username}`}
                style={{
                  fontSize: 12, fontWeight: 700, padding: '4px 14px',
                  borderRadius: 'var(--radius-full)',
                  background: following[current.username] ? 'transparent' : 'var(--blue)',
                  color: following[current.username] ? 'var(--blue-bright)' : '#fff',
                  border: '1px solid var(--blue)',
                  flexShrink: 0,
                }}
              >
                {following[current.username] ? 'Following ✓' : '+ Follow'}
              </button>
            )}
          </div>

          {current.body && (
            <p style={{ fontSize: 15, color: 'var(--text-2)', lineHeight: 1.7, marginBottom: 12, flex: 1 }}>
          ...
          {/* Video player — inline for video posts */}
          {current.type === 'video' && current.audio_url && (
            <div style={{ marginBottom: 16 }}>
              <video
                controls
                src={current.audio_url}
                aria-label={`Video: ${current.title}`}
                style={{ width: '100%', maxHeight: 260, borderRadius: 'var(--radius-sm)', background: '#000', display: 'block' }}
              />
              {current.audio_description && (
                <div style={{ marginTop: 10, padding: '10px 14px', background: 'var(--blue-soft)', border: '1px solid var(--blue)', borderRadius: 'var(--radius-sm)' }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--blue-bright)', marginBottom: 4 }}>🎙️ Audio description</div>
                  <p style={{ fontSize: 13, color: 'var(--text-2)', lineHeight: 1.6 }}>{current.audio_description}</p>
                </div>
              )}
            </div>
          )}

          {/* Primary play control — for audio posts only */}
          {current.type !== 'video' && (
            <button
