      ...
      {current && (
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
            ...
            {echoUser && echoUser.username !== current.username && (
              <button
                onClick={() => handleFollow(current)}
                aria-pressed={!!following[current.username]}
                aria-label={following[current.username] ? `Unfollow ${current.display_name || current.username}` : `Follow ${current.display_name || current.username}`}
                style={{
