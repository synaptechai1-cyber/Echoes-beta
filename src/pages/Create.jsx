...
export default function Create() {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    if (type !== 'moment' && !title.trim()) { setError('Please add a title.'); return }
    if (type === 'moment' && !body.trim()) { setError('What\'s on your mind? Write something.'); return }
    if ((type === 'audio' || type === 'voice_note') && !audioBlob) { setError('Please record your audio first.'); return }
  ...
  function previewText() {
    if (!body) return
    const utterance = new SpeechSynthesisUtterance((title ? title + '. ' : '') + body)
    utterance.rate = 0.95
      ...
      <form onSubmit={handleSubmit} noValidate aria-label="Create a new post">
            ...
            {CONTENT_TYPES.map(ct => (
              <label key={ct.id} style={{
                ...
                <input type="radio" name="type" value={ct.id} checked={type === ct.id}
                  onChange={() => { setType(ct.id); setAudioBlob(null); setAudioPreviewUrl(''); setTitle(''); setBody('') }}
                  className="sr-only" />
                <span style={{ fontSize: 20 }} aria-hidden="true">{ct.icon}</span>
                <span style={{ fontSize: 11, fontWeight: 600, color: type === ct.id ? 'var(--blue-bright)' : 'var(--text)', lineHeight: 1.2 }}>{ct.label}</span>
              </label>
            ))}
          </div>
        </fieldset>

        {/* Moment — casual, no title, just text */}
        {isMoment && (
          <div style={{ marginBottom: 20 }}>
            <label htmlFor="moment_body" className="sr-only">What's on your mind?</label>
            <textarea
              id="moment_body"
              value={body}
              onChange={e => setBody(e.target.value)}
              placeholder="Share a thought, a feeling, a question, anything…"
              rows={5}
              maxLength={500}
              autoFocus
              style={{
                ...
                color: 'var(--text)', fontSize: 16, lineHeight: 1.6, resize: 'none',
              }}
            />
        ...
        {isAudio && (
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--text-3)', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '8px 12px', marginBottom: 12 }}>
              Test mode: keep clips under 2 minutes for best results.
            </div>
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 24, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
              ...
              <p style={{ fontSize: 13, color: 'var(--text-3)' }}>
                {recording ? 'Tap to stop' : audioPreviewUrl ? 'Tap to re-record' : 'Tap to start recording'}
              </p>
            </div>
          </div>
        )}
        ...
        {type === 'written' && (
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              ...
              <button type="button" onClick={previewText} disabled={!body} aria-label="Preview how this will sound"
        ...
        <button type="submit" disabled={submitting}
          aria-label={submitting ? (uploadProgress || 'Publishing…') : 'Publish post'}
          style={{
          ...
          {submitting ? (uploadProgress || 'Publishing…') : isMoment ? 'Share moment' : 'Publish to Echoes'}
        </button>
      </form>
    </div>
  )
