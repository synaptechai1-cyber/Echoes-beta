...
async function handleCreatePost(request, env) {
  ...
  const id = uid()
  await env.DB.prepare(
    `INSERT INTO posts (id, user_id, type, title, body, audio_key, audio_data, audio_mime, duration_seconds, tags, series_id, series_episode)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(id, userId, type, title, content || '', audio_key || '', audio_data || '', audio_mime || 'audio/webm',
    duration_seconds || 0, JSON.stringify(tags || []), series_id || null, series_episode || null).run()
