...
export async function onRequest(context) {
  ...
  if (path === '/notifications/read' && method === 'POST') return handleMarkNotificationsRead(request, env)
  if (path === '/notifications/unread-count' && method === 'GET') return handleUnreadCount(request, env)

  // Direct messages
  if (path === '/conversations' && method === 'GET') return handleGetConversations(request, env)
  if (path === '/conversations' && method === 'POST') return handleStartConversation(request, env)
  if (path.match(/^\/conversations\/[^/]+\/messages$/) && method === 'GET') return handleGetMessages(request, env, path)
  if (path.match(/^\/conversations\/[^/]+\/messages$/) && method === 'POST') return handleSendMessage(request, env, path)
...
async function handleLike(request, env, path) {
  ...
  if (existing) {
    ...
    if (post) await createNotification(env, { userId: post.user_id, actorId: userId, type: 'like', postId, messagePreview: post.title?.slice(0, 60) || '' })
...
async function handleAddComment(request, env, path) {
  ...
  // Notify post author
  const post = await env.DB.prepare('SELECT user_id, title FROM posts WHERE id = ?').bind(postId).first()
  if (post) await createNotification(env, { userId: post.user_id, actorId: userId, type: 'comment', postId, messagePreview: (body || '').slice(0, 60) })
...
async function handleFollow(request, env, path) {
  ...
  if (existing) {
    ...
    await createNotification(env, { userId: target.id, actorId: userId, type: 'follow' })
...
async function createNotification(env, { userId, actorId, type, postId, messagePreview }) {
  if (userId === actorId) return // don't notify yourself
  try {
    await env.DB.prepare(
      `INSERT INTO notifications (id, user_id, actor_id, type, post_id, message_preview)
       VALUES (?, ?, ?, ?, ?, ?)`
    ).bind(uid(), userId, actorId, type, postId || null, messagePreview || '').run()
  } catch {} // never let notification creation break the main action
}
...
async function handleGetNotifications(request, env) {
  ...
  return json({ notifications: results.results })
}
...
async function handleUnreadCount(request, env) {
  ...
  const msgs = await env.DB.prepare(
    `SELECT COALESCE(SUM(CASE WHEN participant_1 = ? THEN unread_1 ELSE unread_2 END), 0) as count
     FROM conversations WHERE participant_1 = ? OR participant_2 = ?`
...
async function handleGetConversations(request, env) {
  ...
  const conversations = results.results.map(c => {
    const isP1 = c.participant_1 === userId
    const other = isP1
      ? { username: c.p2_username, display_name: c.p2_display_name, id: c.participant_2 }
      : { username: c.p1_username, display_name: c.p1_display_name, id: c.participant_1 }
    return {
      id: c.id,
      other_user: other,
      last_message: c.last_message,
      last_message_at: c.last_message_at,
      unread: isP1 ? c.unread_1 : c.unread_2,
    }
  })
...
async function handleStartConversation(request, env) {
  const userId = await getUser(request, env)
  if (!userId) return error('Unauthorized', 401)
  ...
  const other = await env.DB.prepare('SELECT id FROM users WHERE username = ?').bind(username).first()
  if (!other) return error('User not found', 404)
  if (other.id === userId) return error('Cannot message yourself')

  // Ensure consistent ordering so UNIQUE constraint works
  const p1 = userId < other.id ? userId : other.id
  const p2 = userId < other.id ? other.id : userId
...
async function handleGetMessages(request, env, path) {
  const userId = await getUser(request, env)
  if (!userId) return error('Unauthorized', 401)
  ...
  await env.DB.prepare(
    `UPDATE conversations SET ${isP1 ? 'unread_1' : 'unread_2'} = 0 WHERE id = ?`
  ).bind(convId).run()
  ...
  return json({ messages: results.results, conversation: conv })
}

async function handleSendMessage(request, env, path) {
  const userId = await getUser(request, env)
  if (!userId) return error('Unauthorized', 401)
  const convId = path.split('/')[2]
  ...
  ).bind(id, convId, userId, body.trim()).run()

  // Update conversation last message and unread count for recipient
  const isP1 = conv.participant_1 === userId
  await env.DB.prepare(
    `UPDATE conversations SET
       last_message = ?,
       last_message_at = datetime('now'),
       ${isP1 ? 'unread_2' : 'unread_1'} = ${isP1 ? 'unread_2' : 'unread_1'} + 1
     WHERE id = ?`
  ).bind(body.trim().slice(0, 80), convId).run()
  ...
  await createNotification(env, {
    ...
    actorId: userId,
    type: 'message',
    messagePreview: body.trim().slice(0, 60),
  })

  const message = await env.DB.prepare(
    'SELECT m.*, u.username, u.display_name FROM messages m JOIN users u ON m.sender_id = u.id WHERE m.id = ?'
  ).bind(id).first()
