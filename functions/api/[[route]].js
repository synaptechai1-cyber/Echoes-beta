...
async function handleCreatePost(request, env) {
  ...
  const { type, title, content, audio_key, audio_data, audio_mime, duration_seconds, tags, series_id, series_episode } = body
  if (!type) return error('type required')
  if (!['audio','written','voice_note','live','moment'].includes(type)) return error('Invalid post type')
  if (type !== 'moment' && !title) return error('title required')

  // Detect and unfurl a link in the post body, if present. Never blocks
  // or fails post creation — if unfurling fails or times out, the post
  // still publishes, just without a preview card.
  let linkData = null
  ...
  const id = uid()
  await env.DB.prepare(
    `INSERT INTO posts (id, user_id, type, title, body, audio_key, audio_data, audio_mime, duration_seconds, tags, series_id, series_episode, link_url, link_title, link_description, link_image, link_site_name)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(id, userId, type, title, content || '', audio_key || '', audio_data || '', audio_mime || 'audio/webm',
    duration_seconds || 0, JSON.stringify(tags || []), series_id || null, series_episode || null,
...
async function handleAudioUpload(request, env) {
  ...
  // Test mode fallback — D1 storage limit is 2MB per row in some plans and
  // generally not meant for large blobs, so keep this for short test clips.
  if (body.byteLength > 5 * 1024 * 1024) {
    return error('Audio too large for test mode (5MB max while R2 is not connected). Try a shorter recording.')
  }
